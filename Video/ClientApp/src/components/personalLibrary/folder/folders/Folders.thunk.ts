...
export const actionCreators = {
    ...
    loadFolders: (parentFolderId: number, isArchived: boolean, isInit: boolean): AppThunkAction<KnownAction> => async (dispatch, getState) => {
        try {
            ...
            const folders = await httpClient.get<any, FolderVm[]>({url: `${FoldersPaths.all}?IsArchived=${isArchived}&ParentFolderId=${parentFolderId}`} as IHttpClientRequestParameters<any>);
    ...
    getUserRootFolder: (): AppThunkAction<KnownAction> => async (dispatch, getState) => {
        const folder = await httpClient.get<any, FolderVm>({url: FoldersPaths.userRootFolder} as IHttpClientRequestParameters<any>);
    ...
    updateFolder: (model: UpdateFolderVm): AppThunkAction<KnownAction> => async (dispatch, getState) => {
        await httpClient.put<UpdateFolderVm, void>({url: FoldersPaths.update, payload: model} as IHttpClientRequestParameters<any>);
